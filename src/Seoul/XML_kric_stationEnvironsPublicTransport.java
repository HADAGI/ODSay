...
public class XML_kric_stationEnvironsPublicTransport {
...
//itemList : http://apis.data.go.kr/6280000/busStationService/getBusStationIdList?serviceKey=Ob7C7H5BU9u4SqoFiGjEJZKZoIPUKQVKqtlEv2HfIxzE5xBzY2qBNZihHiLrnHZfkDWbvgCxpyepaHJoVhdDjQ%3D%3D&pageNo=1&numOfRows=10&bstopId=163000513
            ...
            for (int j = 0; j < busId.length; j++) { // 버스번호 범위
                ...
                while (true) {//http://apis.data.go.kr/6280000/busStationService/getBusStationIdList?serviceKey=%2Feab3wgUxZR46s%2BAthE7A2Aydqs8Uyq6yUXt3XfJQZ55tF%2BdwhlKZ1mjLQOsE%2BEVBhaUhkOUJATDiam35tA6kA%3D%3D&pageNo=1&numOfRows=10&bstopId=101000002
                    ...
                    API_XML_kric_stationEnvironsPublicTransport apiExplorer_bus = new API_XML_kric_stationEnvironsPublicTransport();
...
//             버스리스트의 갯수
                    ...
                    for (int i = 0; i < nodeList.getLength(); i++) {
                        ...
                        if (node.getNodeType() == Node.ELEMENT_NODE) {
                            ...
                            // 레일포털 서울지하철 - 역사 주변 대중교통 이용정보
                            System.out.println("버스방향 : " + getTagValue("busDir", element));
                            System.out.println("버스노선번호 : " + getTagValue("busRoutNo", element));
                            System.out.println("버스정류장명 : " + getTagValue("busStppNm", element));
                            System.out.println("주요시설명 : " + getTagValue("busStppNo", element));
                            System.out.println("라인 코드 : " + getTagValue("dst", element));
                            System.out.println("철도운영기관코드 : " + getTagValue("exitNo", element));
                            System.out.println("전화번호 : " + getTagValue("lnCd", element));
                            System.out.println("전화번호 : " + getTagValue("railOprIsttCd", element));
                            ...
                            String txt =
                                            getTagValue("busDir", element) + "|" +
                                            getTagValue("busRoutNo", element) + "|" +
                                            getTagValue("busStppNm", element) + "|" +
                                            getTagValue("busStppNo", element) + "|" +
                                            getTagValue("dst", element) + "|" +
...
//                            FileWriter fw = new FileWriter("C:\\Users\\jiwon\\Desktop\\1호선-KR 편의시설.txt", true);
                            FileWriter fw = new FileWriter("C:\\Users\\jiwon\\Desktop\\인천2호선-IC 역사 주변 대중교통 이용정보.txt", true);
                            fw.write(txt);
                            fw.flush();
                            fw.close();
